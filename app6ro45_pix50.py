...
def step_1(base_folder, folder_names):
    ...
    if st.session_state.selected_folder is not None:
        ...
        for i, img_path in enumerate(images):
            ...
            with cols[i % 3]:
                st.image(img, caption=f"이미지 {i + 1}: {image_name}", use_container_width=True)
        ...
        if selected_folder:
            ...
            if st.button("데이터 처리 시작"):
                processed_images = process_images(
                    ...
                    st.session_state.collected_data[selected_folder],
                    resize_option, transformations, not is_grayscale
                )
...
def process_images(folder_key, base_folder, image_names, resize, transformations, is_color=True):
    ...
    for image_name in image_names:
        ...
        for transformation in transformations:
            ...
            elif transformation == "좌우 대칭 및 45도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(45)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 90도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(90)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 135도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(135)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 180도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(180)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 225도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(225)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 270도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(270)
                processed_images.append(np.array(flipped_img))
            elif transformation == "좌우 대칭 및 315도 회전":
                flipped_img = img.transpose(Image.FLIP_LEFT_RIGHT).rotate(315)
                processed_images.append(np.array(flipped_img))
